...
| D21 | review | Convert ID-free app shell identity and new manual connection defaults to WednesdayAI. | Cross-model adversarial review found that app config display was converted while settings version/share/rate/poster surfaces and the create-editor default still presented Clawket or OpenClaw first. These surfaces do not choose final package, store, billing, CLI, or relay identifiers, so they now use WednesdayAI while retaining OpenClaw-compatible auth behaviour. |
| D22 | review | Keep Clawket Pro, Clawket CLI/Bridge, OpenClaw-specific help, and release-bound names unchanged. | Those hits remain tied to billing/product entitlements, current bridge/package/command compatibility, explicit OpenClaw setup guidance, or human-confirmation-bound IDs. Changing them in this slice would broaden scope into unconfirmed migrations. |
| D23 | documentation | Keep the post-execution documentation refresh and record why it is outside task-file implementation drift. | The user explicitly requested documentation coverage after execution. The docs update clarifies current user/admin/developer guidance and self-hosting/release boundaries without changing production identifiers; the round-2 review record now accounts for that divergence. |
| D24 | review | Route in-app GitHub links to the WednesdayAI Mobile repository. | Round-3 adversarial review found the converted app still sent users to `p697/clawket` from Settings and historical release announcements. Repository links are not confirmation-bound production IDs, so the shared app-link constant now points to `ExpansionX/WednesdayAI-mobile`. |
| D25 | review | Preserve per-agent selection for any backend without a global main session. | Round-3 review found WednesdayAI inherited OpenClaw-compatible multi-agent behaviour but App root still reset non-OpenClaw backends to `main`. The guard now uses `resolveGlobalMainSessionKey`, keeping WednesdayAI/OpenClaw per-agent selection and Hermes/YouMind global-main behaviour centralized. |
| D26 | review | Avoid duplicate WednesdayAI poster branding while retaining compatibility co-branding. | Round-3 review found a WednesdayAI connection could render `WednesdayAI x WednesdayAI`. The poster now renders a single WednesdayAI label for the primary backend and keeps compatibility co-branding for OpenClaw, Hermes, and YouMind. |
| D27 | documentation | Align duplicated macOS release docs with the current checked-in native ID. | Round-3 review found both macOS submission guides still instructed maintainers to use `com.p697.clawket`, contradicting the native-ID inventory and iOS release docs. The docs now name the current checked-in `com.expansionx.clawket` while keeping final production ID migration out of scope. |
