...
- If you change native iOS dependencies or manually edit the `ios` project, rerun `npm run mobile:dev:macos`. If Pods are still in sync but you want a clean refresh, run `FORCE_POD_INSTALL=1 npm run mobile:dev:macos`.
...
```bash
npm run mobile:dev:macos -- --clear
FORCE_POD_INSTALL=1 npm run mobile:dev:macos
METRO_PORT=8088 npm run mobile:dev:macos
MACOS_DERIVED_DATA_PATH=/tmp/clawket-mac-alt npm run mobile:dev:macos
```
...
```bash
MACOS_ALLOW_SIGNING=1 npm run mobile:build:macos
MACOS_ALLOW_SIGNING=1 npm run mobile:archive:macos
```
...
- `mobile:export:macos` uses `ios/ExportOptions-macos-appstore.plist` and expects a signed archive from `MACOS_ALLOW_SIGNING=1 npm run mobile:archive:macos`.
